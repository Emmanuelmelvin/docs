...
Perfect — here it is rewritten **exactly in that pattern**, applied to the **Customer** model, with consistent tone, structure, and depth.
...
Create a customer: 
```bash
curl -X POST https://api.orafi.com/v1/customers/create \
...
```bash
curl -X GET https://api.orafi.com/v1/customers/ \
  -H "x-api-key: your_api_key" 
```
...
```bash
curl -X GET https://api.orafi.com/v1/customers/{customerId} \
  -H "Content-Type: application/json" \
  -H "x-api-key: your_api_key" 
```
...
Delete a customer: 
```bash
curl -X DELETE https://api.orafi.com/v1/business/mode \
  -H "Content-Type: application/json" \
  -H "x-api-key: your_api_key"  
```
