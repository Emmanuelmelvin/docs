...
- **Insufficient funds**: Customer sent less than required amount
- **Network congestion**: High fees or slow confirmation
...
async function createPaymentWithRetry(paymentData, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const response = await fetch('/transactions/payment/hosted/create', {
        method: 'POST',
        headers: {
...
2. **Monitor failures**: Set up alerts for high error rates
3. **Graceful degradation**: Handle failures without breaking user experience
4. **User communication**: Inform users of payment issues clearly
5. **Retry limits**: Don't retry indefinitely
6. **Circuit breakers**: Stop retrying if service is down